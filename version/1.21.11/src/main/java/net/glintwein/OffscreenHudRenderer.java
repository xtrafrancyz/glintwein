...
public class OffscreenHudRenderer {
    ...
    public static int getFbo(RenderTarget target) {
        GlDevice backend = (GlDevice) RenderSystem.getDevice();
        return ((GlTextureView) target.getColorTextureView()).getFbo(backend.directStateAccess(), target.getDepthTexture());
    }
