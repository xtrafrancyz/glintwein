...
public class Yoga3_3_3 implements YogaShim {
    @Override
    ...
    public void NodeMarkDirtyAndPropogateToDescendants(long node) {
        Yoga.YGNodeMarkDirtyAndPropagateToDescendants(node);
    }

    @Override
    public NativeResource NodeSetMeasureFunc(long node, YogaMeasureFunction measureFunc) {
        YGMeasureFunc nativeMeasureFunc = YGMeasureFunc.create((node0, width, widthMode, height, heightMode, __result) -> {
            Size result = measureFunc.measure(width, sizeModeFromYoga(widthMode), height, sizeModeFromYoga(heightMode));
            __result.set(result.getWidth(), result.getHeight());
        });
